...
export function validateNetworkAction(input: {
  ...
  if (state.activeSeats && !state.activeSeats.includes(seat)) {
    return { ok: false, reason: 'Inactive seat cannot submit actions.' };
  }
  ...
  if (action.type === 'discard') {
    if (state.phase !== 'playing') {
      return { ok: false, reason: `Cannot discard during phase: ${state.phase}` };
    }
    if (state.currentSeat !== seat) {
      return { ok: false, reason: `It is player ${state.currentSeat}'s turn, not yours.` };
    }

    const player = state.players.find((p: any) => p.seat === seat);
    if (!player) {
      return { ok: false, reason: 'Player session not found in state.' };
    }

    const hasTile = player.hand.some((t: any) => t.instanceId === action.tileInstanceId);
    if (!hasTile) {
      return { ok: false, reason: 'You do not hold this tile in hand.' };
    }
  ...
  if (state.phase !== 'waitingForResponses') {
    return { ok: false, reason: 'Not currently waiting for response actions.' };
  }

  const actionsForSeat = state.pendingActions.filter((pa: any) => pa.seat === seat);
  if (actionsForSeat.length === 0) {
    return { ok: false, reason: 'No pending actions for your seat.' };
  }

  if (action.type === 'pass') {
    const hasPass = actionsForSeat.some((pa: any) => pa.type === 'pass');
    if (!hasPass) return { ok: false, reason: 'No pending pass action.' };
    return { ok: true };
  }

  if (action.type === 'hu') {
    const hasHu = actionsForSeat.some((pa: any) => pa.type === 'hu' || pa.type === 'ziMo');
    if (!hasHu) return { ok: false, reason: 'No pending hu or ziMo action.' };
    return { ok: true };
  }

  if (action.type === 'peng') {
    const hasPeng = actionsForSeat.some((pa: any) => pa.type === 'peng');
    if (!hasPeng) return { ok: false, reason: 'No pending peng action.' };
    return { ok: true };
  }

  if (action.type === 'chi') {
    const pendingChi = actionsForSeat.find((pa: any) => pa.type === 'chi');
    if (!pendingChi) return { ok: false, reason: 'No pending chi action.' };
    ...
    if (pendingChi.options) {
      ...
      if (!optionValid) {
        return { ok: false, reason: 'Invalid chi option selected.' };
      }
    }
    return { ok: true };
  }
  ...
  if (action.type === 'gang') {
    ...
    if (!pendingGang) return { ok: false, reason: 'No pending gang action.' };
    ...
    if (pendingGang.options) {
      ...
      if (!optionValid) {
        return { ok: false, reason: 'Invalid gang option selected.' };
      }
    }
    return { ok: true };
  }
